...
class WPUPG_Giveaway {
    ...
    public function giveaway_notice()
    {
        ...
        if ( 'edit-wpupg_grid' === $screen->id ) {
			...
			echo '<strong>Feeling lucky?</strong> Win plugins in our <a href="' . esc_url( admin_url( 'edit.php?post_type=' . WPUPG_POST_TYPE . '&page=wpupg_giveaway' ) ) . '" target="_blank">Birthday Giveaway</a>!';
			echo '</div>';
        }
    }
}