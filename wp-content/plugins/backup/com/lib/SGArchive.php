...
class SGArchive
{
	...
	private function extractFiles($destinationPath)
	{
		...
		while ($this->cdrFilesCount) {
			...
			if($restoreMode == SG_RESTORE_MODE_FULL || ($restoreMode == SG_RESTORE_MODE_DB && backupGuardStringEndsWith($path,$sqlFileEnding)) || ($restoreMode == SG_RESTORE_MODE_FILES && !backupGuardStringEndsWith($path,$sqlFileEnding))) {
